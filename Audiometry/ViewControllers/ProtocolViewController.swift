...
//------------------------------------------------------------------------------
    ...
    private var _currentCalibrationSetting: TestSetting! = nil
    ...
    private var _testLanguage: TestLanguage = .english
...
//------------------------------------------------------------------------------
    ...
    @IBOutlet weak var pbAdultTest: UIButton!
    @IBOutlet weak var pbChildrenTest: UIButton!
...
//        }
        ...
        _currentCalibrationSetting = setting
    ...
    func loadProtocol(){
        _currentCalibrationSetting = _calibrationSettings[_currentPickerIndex]
        _testSequenceFrequencies = _currentCalibrationSetting.frequencySequence ?? []
    ...
    @IBAction func deleteFreqSeqProtocol(_ sender: UIButton) {
        ...
        _managedContext.delete(_currentCalibrationSetting)
        _currentCalibrationSetting = nil
...
//------------------------------------------------------------------------------
    ...
    @IBAction func addNewTestSequenceFrequency(_ sender: UIButton){
        let frequency: Int! = sender.tag
        if(!_testSequenceFrequencies.contains(frequency) ){
            _testSequenceFrequencies.append(frequency)
            reloadTestSequenceLabel()
        }
    }
    
    @IBAction func removeLastTestSequenceFrequency(_ sender: UIButton) {
        if(_testSequenceFrequencies.count > 0) {
    ...
    @IBAction func removeAllTestSequenceFrequencies(_ sender: UIButton) {
        if(_testSequenceFrequencies.count > 0) {
...
//------------------------------------------------------------------------------
    ...
    func savePatientProfile(_ patientGroup: String,
        ...
        let totalTestCount: Int = _globalSetting.isTestingBoth
            ? _testSequenceFrequencies.count*2
            : _testSequenceFrequencies.count
        _globalSetting.totalTestCount = Int16(totalTestCount)
    ...
    private func setupUI(){
        ...
        for frequency in DEFAULT_FREQUENCIES {
            ...
            // i.e. to play signal
            pbNewFrequencyToggle.addTarget(
                self, action: #selector(addNewTestSequenceFrequency(_:)),
                for: .touchUpInside)
    ...
    private func reloadLocaleStrings() {
        ...
        pbAdultTest.setTitle(
            NSLocalizedString("Adult Test", comment: ""), for: .normal)
        pbChildrenTest.setTitle(
            NSLocalizedString("Children Test", comment: ""), for: .normal)
    }
